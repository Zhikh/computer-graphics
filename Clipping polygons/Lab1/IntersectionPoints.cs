...
namespace Lab1
{
    class IntersectionPoints
    {
        ...
        private double FindMainDeterminant(SDL.SDL_Point A, SDL.SDL_Point B, SDL.SDL_Point C, SDL.SDL_Point D)
        {
            return (B.x - A.x) * (C.y - D.y) - (C.x - D.x) * (B.y - A.y);
        }

        private double FindMuDeterminant(SDL.SDL_Point A, SDL.SDL_Point B, SDL.SDL_Point C, SDL.SDL_Point D)
        {
            return (C.x - A.x) * (C.y - D.y) - (C.x - D.x) * (C.y - A.y);
        }

        private double FindLambdaDeterminant(SDL.SDL_Point A, SDL.SDL_Point B, SDL.SDL_Point C, SDL.SDL_Point D)
        {
            return (B.x - A.x) * (C.y - A.y) - (B.y - A.y) * (C.x - A.x);
        }

        public bool IsSegmentsIntersect(SDL.SDL_Point A, SDL.SDL_Point B, SDL.SDL_Point C, SDL.SDL_Point D)
        {
            double delta = FindMainDeterminant(A, B, C, D);
            if (delta != 0)
            {
                double mu = FindMuDeterminant(A, B, C, D) / delta;
                double lambda = FindLambdaDeterminant(A, B, C, D) / delta;
        ...
        private void Intersection(SDL.SDL_Point A, SDL.SDL_Point B, SDL.SDL_Point C, SDL.SDL_Point D)
        {
            ...
            crossingPoint.x = (int)Math.Floor((xo * q * p1 - x1 * q1 * p - yo * p * p1 + y1 * p * p1) /
                (q * p1 - q1 * p));
            crossingPoint.y = (int)Math.Floor((yo * p * q1 - y1 * p1 * q - xo * q * q1 + x1 * q * q1) /
                (p * q1 - p1 * q));
        }
