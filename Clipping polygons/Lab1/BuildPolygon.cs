...
namespace Lab1
{
    ...
    {
        public static void Build_RightPolygon(RightPolygonData cicle, double heightScale, int offsetX, int offsetY)
        {
            ...
            for (i = 0; i < cicle.N; i++)
            {
                points[i].x = (int)Math.Floor((cicle.X0 + cicle.R * Math.Cos(omega * i + cicle.Phi)) * heightScale + offsetX);
                points[i].y = (int)Math.Floor((cicle.Y0 - cicle.R * Math.Sin(omega * i + cicle.Phi)) * heightScale + offsetY);
            }
            cicle.Points = points;
        }

        public static void Build_Rectangle(RectangleData rectangle, double widthScale, double heightScale, int offsetX, int offsetY)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                points[i].x = (int)Math.Floor(points[i].x * widthScale + offsetX);
                points[i].y = (int)Math.Floor(points[i].y * heightScale + offsetY);
            }
